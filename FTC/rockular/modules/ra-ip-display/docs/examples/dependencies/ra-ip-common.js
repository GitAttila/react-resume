...
angular.module("RA.IPCommon.Services").factory("fqnSvc", [ "raErrorHandlerSvc", function(raErrorHandlerSvc) {
    ...
    /**
         ...
         * @param {string} fqn The fully qualified name
         * @param {string} parameter string to search for
         * @returns {number} -1 if parameter doesn't exists or number when parameter starts
         */
 ...
 * @ngdoc service
 * @module RA.IPCommon.Services
 * @name RA.IPCommon.Services.service:simpleSubscriberSvc
 * @description
 * Simplified version of subscriberSvc.
 * See {@link RA.IPCommon.Services.service:subscriberSvc SubscriberSvc}
 **/
angular.module("RA.IPCommon.Services").service("simpleSubscriberSvc", [ "$q", "subscriberSvc", "raErrorHandlerSvc", function($q, subscriberSvc, raErrorHandlerSvc) {
    "use strict";
    var self = this;
    ...
    /**
             * @ngdoc method
             * @module RA.IPCommon.Services
             * @name RA.IPCommon.Services.service:simpleSubscriberSvc#subscribePromise
             * @methodOf RA.IPCommon.Services.service:simpleSubscriberSvc
             * @description
             * @description Method as helper to subscriberSvc, subscribes for repeated data base on parameters.
             * @param {String|Integer} id unique id for repeated data, serves as registration key for consumer in
             * subscriberSvc
             * @param {Number} defaultUpdateInterval interval in seconds for $interval used in subscriberSvc, default
             * and maximum update interval
             * @param {Function} subscriptionPromiseFn function to be called repeatedly by given updateInterval,
             * this function have to return a promise
             * @param {Number} failedAttemptsCount optional count to try repeated request when they fails
             *  @param {Number} startAtUpdateIntervalOf ptional parameter to apply firs next update at given seconds,
             *  this have to be less then updateInterval, instead updateInterval is used
             *  @param {Number} increaseByNumberOfSeconds optional parameter to apply interval increase
             *  up to the default maximum updateInterval given, use it with startAtUpdateIntervalOf parameter
             * @returns {promise}
             * - notify on promise is called repeatedly
             * - reject is called when there is error case and subscriber is unsubscribed
             
             * @throws {RAError} exception when there are wrong parameters
             */
    self.subscribePromise = function(id, defaultUpdateInterval, subscriptionPromiseFn, failedAttemptsCount, startAtUpdateIntervalOf, increaseByNumberOfSeconds) {
        var deferred = $q.defer(), failedAttempts = 0, lastUpdateInterval, updateInterval, firstRepeatedRun = true;
        if (!angular.isNumber(defaultUpdateInterval) || !angular.isDefined(id) || !angular.isDefined(subscriptionPromiseFn)) {
            throw new raErrorHandlerSvc.RAError("Invalid argument", "Invalid argument has been passed to a function.");
        }
        updateInterval = defaultUpdateInterval * 1e3;
        lastUpdateInterval = updateInterval;
        if (angular.isNumber(startAtUpdateIntervalOf) && startAtUpdateIntervalOf < defaultUpdateInterval) {
            lastUpdateInterval = startAtUpdateIntervalOf * 1e3;
        }
        function getInterval() {
            var increaseResult;
            var interval = lastUpdateInterval;
            if (angular.isNumber(increaseByNumberOfSeconds) && !firstRepeatedRun) {
                increaseResult = interval + increaseByNumberOfSeconds * 1e3;
                if (increaseResult < updateInterval) {
                    interval = increaseResult;
                } else {
        ...
        function onUpdate() {
            self.clearTimer();
            subscriptionPromiseFn().then(function(response) {
                failedAttempts = 0;
                deferred.notify(response);
                self.startTimer(getInterval());
                firstRepeatedRun = false;
            }, function(reason) {
                failedAttempts++;
                if (angular.isNumber(failedAttemptsCount)) {
                    if (failedAttempts >= failedAttemptsCount) {
                        self.unsubscribe(id, reason, true);
        ...
        self.subscribe(id, {}, {}, null, null, onUpdate);
        self.update(id);
        // trigger update to begin with repeated data calls, calls onUpdate fn above
        return deferred.promise;
    };
    subscriberSvc.extend(self);
} ]);
