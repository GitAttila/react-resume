...
/** MFT Change
 * @description
 * This directive is responsible for showing the version picker on the documentation pages.
 * It assumes, that different versions of the documentation are placed in the directory one level up,
 * and their folder names start with "v". It calls the URL "/api/docVersions" to get the JSON array
 * with the names of the folders in the upper directory.
 */
docsApp.directive.versionPicker = function($http, $window) {
  return {
    template: "<select class='mft-version-picker' ng-show='options.length' ng-model='versionSelect'" +
              " ng-change='jumpToVersion()' ng-options='option.tag as option.name for option in options'>" +
              "</select>",
    link: function (scope) {
      // get an array of path elements on the left side of the hash
      var pathElements = $window.location.href.split("#")[0].split("/");

      if (pathElements.length > 1) {
        // get the last part of the path (-2, because the last element is always empty)
        scope.versionSelect = pathElements[pathElements.length - 2];
      }

      // call the API to get the folder names
      $http.get("/docs-versions/").then(function(response) {
        if (response && response.data && response.data.length > 1) {
          scope.options = [];
          response.data.forEach(function (file) {
            // pick only names starting with "v"
            if (file.type === "directory" && file.name[0] === "v") {
              scope.options.unshift({
                tag: file.name,
                name: file.name
              });
            }
            // for node json output format
            else if (angular.isString(file) && file[0] === "v") {
              scope.options.unshift({
                tag: file,
                name: file
              });
          ...
          if (scope.options.length) {
            // if the current directory does not start with "v", pick the first one possible
            // this is the case, when we are in the "latest" folder
            if (scope.versionSelect[0] !== "v") {
              scope.versionSelect = scope.options[0].tag;
            }

            // add information about the latest version possible
            scope.options[0].name += " (latest)";
          }
        }
      });

      // redirect the browser to the folder of the chosen version, maintaining URL after the hash
      scope.jumpToVersion = function() {
        var route = $window.location.href.split("#");
        $window.location.href = '/docs/' + scope.versionSelect + '/#' + route[1];
      }
    }
  };
};
...
docsApp.directive.sourceEdit = function(getEmbeddedTemplate) {
  return NG_DOCS.editExample ? {
    ...
    scope: true,
    controller: function($scope, $attrs, openPlunkr, $http) {
      var sources = {
      ...
      $http.get('examples/dependencies.json')
        .then(function(res){
          sources.exampleDeps = res.data;
        });
...
// MFT added
docsApp.directive.iframeAdjustDimensions = function() {
    return {
        restrict: 'A',
...
docsApp.serviceFactory.loadedUrls = function($document) {
  ...
  angular.forEach($document.find('link'), function(script) {
    ...
    if (match) {
        styles[match[1].replace(/(\-\d.*)?(\.min)?\.css$/, '.css')] = match[0];
    }
  });
  /*MFT change end*/
  ...
  urls.baseStyles = [];
  angular.forEach(NG_DOCS.scripts, function(script) {
    var match = styles[script.replace(/(\-\d.*)?(\.min)?\.css$/, '.css')];
    if (match) {
        urls.baseStyles.push(match);
    }
  });
  /*MFT change end*/
  return urls;
...
docsApp.serviceFactory.openPlunkr = function(templateMerge, formPostData, loadedUrls, $location) {
  return function(content) {
    ...
    /*MFT change begin*/
    angular.forEach(content.exampleDeps.scripts, function(url) {
        scriptDeps += '    <script src="' + baseUrl + 'examples/dependencies/' + url + '"></script>\n';
    });

    angular.forEach(content.exampleDeps.styles, function(url) {
        scriptDeps += '    <link rel="stylesheet" href="' + baseUrl + 'examples/dependencies/' +
            url + '" type="text/css">\n';
    });
    /*MFT change end*/
    ...
    angular.forEach(allFiles, function(file, index) {
      if (file.name == 'index-new-window.html') { // MFT Change
